...
Shader "FAE/Tree Trunk New"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 _MainTex_ST, _BumpMap_ST;
			float _GradientBrightness, _UseSpeedTreeWind, _Smoothness;
			#ifdef _TRANSMISSION_ASE
				float _TransmissionShadow;
			#endif
			#ifdef _TRANSLUCENCY_ASE
				float _TransStrength, _TransNormal, _TransScattering, _TransDirect, _TransAmbient, _TransShadow;
			#endif
			#ifdef TESSELLATION_ON
				float _TessPhongStrength, _TessValue, _TessMin, _TessMax, _TessEdgeLength, _TessMaxDisp;
			#endif
			CBUFFER_END
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
                ...
                half2 uv1 = half2(IN.ase_texcoord7.zw);
				// half2 texCoord134 = IN.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
                half2 uv2 = half2(IN.ase_texcoord8.xy);
				half4 lerpColor = lerp( ( _GradientBrightness * mainColor ) , mainColor , (( _UseSpeedTreeWind ) ? ( uv2.y ):( ( 1.0 - uv1.y ) )));
				...
				half roughness = ( mainColor.a * _Smoothness );
				
				float3 Albedo = lerpColor.rgb;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
                ...
                half2 uv1 = half2(IN.ase_texcoord2.zw);
				// half2 texCoord134 = IN.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
                half2 uv2 = half2(IN.ase_texcoord3.xy);
				half4 lerpColor = lerp((_GradientBrightness * mainColor), mainColor, ((_UseSpeedTreeWind) ? (uv2.y) : ((1.0 - uv1.y))));
