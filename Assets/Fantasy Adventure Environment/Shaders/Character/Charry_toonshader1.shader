...
//带顶点飘动动画的角色shader
{
    ...
    SubShader
    {
        ...
        float GaussianBlur[9] = {0.0947416f, 0.118318f, 0.0947416f, 0.118318f, 0.147761f, 0.118318f, 0.0947416f, 0.118318f, 0.0947416f};

        // half4 Gaussian_Blur(Texture2D tex, sampler2D samplerX, float2 uv){
            //     half4 outcol;
            //     float offset = 1/16;

            //     outcol = SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x - offset, uv.y - offset)) * GaussianBlur[0];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x, uv.y - offset)) * GaussianBlur[1];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x + offset, uv.y - offset)) * GaussianBlur[2];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x - offset, uv.y)) * GaussianBlur[3];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x, uv.y)) * GaussianBlur[4];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x + offset, uv.y)) * GaussianBlur[5];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x - offset, uv.y + offset)) * GaussianBlur[6];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x, uv.y + offset)) * GaussianBlur[7];
            //     outcol += SAMPLE_TEXTURE2D(tex, samplerX, float2(uv.x + offset, uv.y + offset)) * GaussianBlur[8];
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                float offset = 0.015f / 16;

                half4 outlineTex = SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x - offset, i.uv.y - offset)) * 0.0947416f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x, i.uv.y - offset)) * 0.118318f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x + offset, i.uv.y - offset)) * 0.0947416f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x - offset, i.uv.y)) * 0.118318f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x, i.uv.y)) * 0.147761f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x + offset, i.uv.y)) * 0.118318f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x - offset, i.uv.y + offset)) * 0.0947416f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x, i.uv.y + offset)) * 0.118318f;
                outlineTex += SAMPLE_TEXTURE2D(_OutlineTex, sampler_OutlineTex, float2(i.uv.x + offset, i.uv.y + offset)) * 0.0947416f;
