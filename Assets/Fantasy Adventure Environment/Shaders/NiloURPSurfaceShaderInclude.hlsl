...
UserGeometryOutputData BuildUserGeometryOutputData(Attributes IN, bool isExtraCustomPass = false)
{
    ...
    outputData = (UserGeometryOutputData)0;
...
UserSurfaceOutputData BuildUserSurfaceOutputData(Varyings IN, bool isExtraCustomPass)
{
    ...
    surfaceData.specular = 0;               //default black  
    ...
    surfaceData.specular = max(0,surfaceData.specular);
    return surfaceData;
}
...
half4 CalculateSurfaceFinalResultColor(Varyings IN, UserSurfaceOutputData surfaceData,  FAELightingData lightingData);
// 兰伯特模型的着色函数
half4 CalculateSurfaceFinalResultColorLambert(Varyings IN, UserSurfaceOutputData surfaceData,  FAELightingData lightingData);
...
half4 fragAllWork(Varyings IN, bool shouldOnlyDoAlphaClipAndEarlyExit = false, bool isExtraCustomPass = false)
{
    ...
    half3 reflectionDirectionWS = reflect(-viewDirectionWS, lightingData.normalWS);
        ...
        lightingData.shadowMask = SAMPLE_SHADOWMASK(IN.uv2);
    #else
        lightingData.shadowMask = half4(1, 1, 1, 1);
    #endif
    //return lightingData.shadowMask;
    // shadowCoord is position in shadow light space (must compute in fragment shader after URP 7.X)
    float4 shadowCoord = TransformWorldToShadowCoord(positionWS);
    lightingData.mainDirectionalLight = GetMainLight(shadowCoord,positionWS,lightingData.shadowMask);
    ...
    float2 normalizedScreenSpaceUV = GetNormalizedScreenSpaceUV(IN.positionCS);
        ...
        lightingData.directAmbientOcclusion = aoFactor.directAmbientOcclusion;
    #endif
...
half4 fragUniversalForwardLambert(Varyings IN) : SV_Target
{
    //re-normalize all directions vector after interpolation
    IN.normalWS = normalize(IN.normalWS);
    IN.tangentWS.xyz = normalize(IN.tangentWS);
    ...
    //TODO: what if user want postprocess both before fog and after?
    half4 finalColor = CalculateSurfaceFinalResultColorLambert(IN, surfaceData, lightingData);
    FinalPostProcessFrag(IN, surfaceData, lightingData, finalColor);
