...
Shader "Babeitime/Scene/RealisticWaterWithDistortion"
{
	Properties
	{
		...
		_SurfaceDistortionAmountNormal("Surface Distortion Amount(Water Normal)", Range(0, 1)) = 0.27
		_SurfaceDistortionAmount("Surface Distortion Amount(Form)", Range(0, 1)) = 0.27
	}
...
/* UNITY: Original start of shader */
		...
		float _SurfaceDistortionAmountNormal;
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			float2 distortSample = (tex2D(_SurfaceDistortion, i.worldPos.xz * _SurfaceDistortion_ST.xy + _SurfaceDistortion_ST.zw).xy * 2 - 1);
			float2 waterNormalDistortion = distortSample * _SurfaceDistortionAmountNormal;
			float2 foamDistortion = distortSample * _SurfaceDistortionAmount;

			float3 lerpResult224 = lerp( UnpackNormal( tex2D( _WaterNormal, temp_output_568_0 + waterNormalDistortion ) ) , UnpackNormal( tex2D( _WaterNormal, temp_output_568_9 + waterNormalDistortion ) ) , 0.5);
			float2 temp_output_486_0 = ( temp_output_568_0 * 0.15 );
			float2 temp_output_487_0 = ( temp_output_568_9 * 0.15 );
			float3 lerpResult489 = lerp( UnpackNormal( tex2D( _WaterNormal, temp_output_486_0 + waterNormalDistortion ) ) , UnpackNormal( tex2D( _WaterNormal, temp_output_487_0 + waterNormalDistortion ) ) , 0.5);
			...
			float lerpResult74 = lerp( tex2D( _Foam, panner1_g64 + foamDistortion ).r , tex2D( _Foam, panner2_g64 + foamDistortion ).r , 0.5);
...
//			float4 blendOpSrc138 = ( lerp(float4( indirectSpecular261 , 0.0 ),tex2D( _ReflectionTex, ( ( (lerpResult283).xy * _ReflectionDistortion ) + appendResult525 ) ),_UseScriptReflection) * saturate( fresnelNode256 ) * ase_lightAtten );
			...
			c.a = lerp(distanceDepth100, 1, temp_cast_5);
			return c;
		}
