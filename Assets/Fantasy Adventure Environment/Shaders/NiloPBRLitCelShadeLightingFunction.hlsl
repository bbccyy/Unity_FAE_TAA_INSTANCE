...
half4 CalculateSurfaceFinalResultColor(Varyings IN, UserSurfaceOutputData surfaceData, FAELightingData  lightingData)
{
    ...
    {
        Light light = GetAdditionalLight(i,lightingData.positionWS,lightingData.shadowMask);
...
half3 LightingLambertCelShade(UserSurfaceOutputData surfaceData, Light light, half3 normalWS, half3 viewDirectionWS)
{
    ...
    return surfaceData.albedo * radiance;
}
...
half4 CalculateSurfaceFinalResultColorLambert(Varyings IN, UserSurfaceOutputData surfaceData, FAELightingData  lightingData)
{
    ...
    // LightingPhysicallyBased computes direct light contribution.
    //this line adds main directional light's contribution 
    rgb += LightingLambertCelShade(surfaceData, lightingData.mainDirectionalLight, lightingData.normalWS, lightingData.viewDirectionWS);
    ...
    {
        Light light = GetAdditionalLight(i,lightingData.positionWS,lightingData.shadowMask);
        rgb += LightingLambertCelShade(surfaceData, light, lightingData.normalWS, lightingData.viewDirectionWS);
    }
